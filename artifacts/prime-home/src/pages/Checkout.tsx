...
export default function Checkout() {
  ...
  const [form, setForm] = useState({
    ...
    deliveryArea: "",
    notes: "",
    paymentMethod: "cash_on_delivery" as "cash_on_delivery" | "whatsapp",
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        ...
        >
          ...
          <p className="text-muted-foreground mb-8">Total: <span className="font-semibold text-foreground">${orderPlaced.total.toFixed(2)}</span></p>
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-3 space-y-6">
            ...
            <div className="rounded-2xl border bg-card p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label>City *</Label>
                  <Input value={form.city} onChange={(e) => setField("city", e.target.value)} required placeholder="Beirut" className="rounded-xl" />
                </div>
                <div className="space-y-2">
            ...
            <div className="rounded-2xl border bg-card p-6">
              ...
              <RadioGroup value={form.paymentMethod} onValueChange={(v) => setField("paymentMethod", v)} className="space-y-3">
                ...
                <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${form.paymentMethod === "whatsapp" ? "border-primary bg-primary/5" : ""}`}>
                  <RadioGroupItem value="whatsapp" id="whatsapp" />
                  <span className="text-lg">💬</span>
                  <div>
                    <p className="font-medium text-sm">WhatsApp Order</p>
                    <p className="text-xs text-muted-foreground">We'll confirm your order via WhatsApp</p>
                  </div>
                </label>
              </RadioGroup>
            </div>
