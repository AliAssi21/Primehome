...
router.get("/products", async (req, res): Promise<void> => {
  ...
  if (minPrice != null) {
    conditions.push(gte(sql`cast(coalesce(${productsTable.salePrice}, ${productsTable.price}) as decimal(10,2))`, minPrice));
  }
  if (maxPrice != null) {
    conditions.push(lte(sql`cast(coalesce(${productsTable.salePrice}, ${productsTable.price}) as decimal(10,2))`, maxPrice));
  }
...
router.post("/products", requireAdmin, async (req, res): Promise<void> => {
  ...
  if (parsed.data.categoryId) {
    ...
    if (!category) {
      res.status(400).json({ error: `Category ${parsed.data.categoryId} not found` });
      return;
    }
  }
  ...
  const [product] = await db.select().from(productsTable).where(eq(productsTable.slug, data.slug));
...
router.patch("/products/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  await db.update(productsTable).set(data).where(eq(productsTable.id, params.data.id));
