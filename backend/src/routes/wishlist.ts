...
import { db, wishlistTable, productsTable, categoriesTable } from "../../db";
import { AddToWishlistParams, RemoveFromWishlistParams } from "../../api-zod";
...
router.get("/wishlist", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json(rows.map((r: typeof rows[0]) => formatProduct({ ...r.p }, r.categoryName)));
});
...
router.post("/wishlist/:productId", requireAuth, async (req, res): Promise<void> => {
  ...
  await db.insert(wishlistTable).ignore().values({ userId: req.user!.userId, productId: params.data.productId });
