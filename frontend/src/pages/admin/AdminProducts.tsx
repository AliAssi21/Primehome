...
import { Plus, Search, Pencil, Trash2, Eye, EyeOff, Loader2, Upload, X } from "lucide-react";
...
export default function AdminProducts() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        setField("images", currentImages.join("\n"));
      }
  ...
  const handleAddManualUrl = () => {
    ...
    const currentImages = form.images.split("\n").map((s) => s.trim()).filter(Boolean);
    currentImages.push(manualUrl.trim());
    setField("images", currentImages.join("\n"));
    setManualUrl("");
  ...
  const handleRemoveImage = (indexToRemove: number) => {
    ...
    setField("images", updatedImages.join("\n"));
  };
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 space-y-2">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                  {form.images.split("\n").map((s) => s.trim()).filter(Boolean).map((url, idx) => (
                    <div key={idx} className="relative aspect-square rounded-xl overflow-hidden bg-muted border border-border group">
                      ...
                      <button
                        ...
                        className="absolute top-1.5 right-1.5 h-6 w-6 rounded-full bg-black/60 hover:bg-black/80 flex items-center justify-center text-white transition-colors duration-200"
                      >
                  ...
                  {/* Upload card */}
                  <label className="relative aspect-square rounded-xl border-2 border-dashed border-muted-foreground/20 hover:border-primary/50 transition-colors flex flex-col items-center justify-center gap-1.5 cursor-pointer bg-muted/20 group">
                    <input
                      type="file"
                      accept="image/*"
                      multiple
                    ...
                    {uploading ? (
                      ...
                      <>
                        ...
                        <span className="text-[10px] text-muted-foreground group-hover:text-primary transition-colors font-medium">Upload Pic</span>
                      </>
                    )}
                  </label>
                </div>

                {/* Manual URL entry field (backup) */}
