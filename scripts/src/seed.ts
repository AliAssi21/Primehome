import { db, usersTable, categoriesTable, productsTable, settingsTable, couponsTable } from "@workspace/db";
...
async function seed() {
  ...
  console.log("Admin user created: admin@primehome.lb / admin123");
  ...
  for (const product of productData) {
    await db.insert(productsTable).values(product as typeof product & { images: string[] }).onConflictDoNothing();
  }
